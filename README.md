...
7. 画面いっぱいに映したいときは「全画面」を押す（Esc で解除）
8. 一時的にカメラを止めたいときは「カメラ: ON」を押す（もう一度押すと再開）

### カメラのオン・オフ

「カメラ: ON / OFF」はカメラのトラックを `stop()` してデバイスを解放します。SpoutCam の接続が切れ、タブのカメラ表示（赤丸）も消えるので、他のアプリにカメラを譲りたいときに使えます。

オフの間は canvas を黒で塗り続けるため、PiP ウィンドウは開いたまま黒くなります。自動 PiP はカメラを capture 中のページにしか発火しないので、オフの間は働きません。オフのままカメラを選び直した場合は、次にオンにしたときにそのカメラで起動します。オフ状態は保存されず、開き直すと必ずオンで始まります。
...
画面右上に `v1.4.0 · 2026-08-28 10:29` のように、バージョンと配信ファイルの更新時刻を表示します。リポジトリへのリンクになっています。時刻は `document.lastModified` から取っており、GitHub Pages ではデプロイ時刻になるため、番号の付け替えを忘れてもデプロイが反映されたか判別できます。
